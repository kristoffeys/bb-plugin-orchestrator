...
export class OrchestratorStore {
  ...
  private updateAggregatedUsage(coordinatorThreadId: string) {
    ...
    const run = this.getRun(coordinatorThreadId);
    if (run !== null) this.db.prepare("UPDATE orchestration_sessions SET total_tokens = ?, coordinator_tokens = ?, input_tokens = ?, cached_input_tokens = ?, output_tokens = ?, reasoning_output_tokens = ?, updated_at = ? WHERE session_id = ?")
      .run(aggregate.total, coordinator?.total ?? 0, aggregate.input, aggregate.cached, aggregate.output, aggregate.reasoning, Date.now(), run.sessionId);
